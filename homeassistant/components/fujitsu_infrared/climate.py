...
from .entity import FujitsuIrEntity, config_entry_protocol
...
class FujitsuAcClimateEntity(
    ...
    def __init__(self, entry: ConfigEntry, emitter_entity_id: str) -> None:
        ...
        self._protocol = config_entry_protocol(entry)
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if (fan_mode := attributes.get(ATTR_FAN_MODE)) in _HA_FAN_TO_LIB:
    ...
    async def _async_send_state(
        ...
        *,
        ...
        if temperature is None:
            temperature = self._attr_target_temperature or MIN_TEMP
        ...
        await self._send_command(
            FujitsuAcCommand(
                ...
                power=power,
                mode=mode,
                temperature=self._snap_temperature(temperature),
    ...
    def _snap_temperature(self, temperature: float) -> float:
        """Round a temperature to a step the configured format can carry exactly.

        Home Assistant does not enforce the step, and the whole-degree format has no
        field to put a half in.
        """
